...
int main(int argc, char *argv[]) {
    ...
    for (int patchIdx = 0; patchIdx < numPatches; ++patchIdx) {
        ...
        checkCudaError(cudaMalloc(TODO));
        checkCudaError(cudaMalloc(TODO));
    }
    ...
    for (int deviceIdx = 0; deviceIdx < numDevices; ++deviceIdx) {
        ...
        checkCudaError(cudaMemcpy(
            TODO,                       // destination - local to the patch
            TODO,                       // source - one large host array
            TODO,                       // size including halos
    ...
    auto gatherTemperature = [&]() {
        ...
        for (int deviceIdx = 0; deviceIdx < numDevices; ++deviceIdx) {
            ...
            checkCudaError(cudaMemcpy(
                TODO,                       // destination - a slice in the global host array; skip halo row
                TODO,                       // source - local to the patch; skip top halo
                TODO,                       // size excluding top and bottom halo rows (but including left/right halos)
    ...
    auto work = [&](size_t it) {
        for (int deviceIdx = 0; deviceIdx < numDevices; ++deviceIdx) {
            ...
            stencil2D<<<patch.gridSize, patch.blockSize>>>(
                TODO,                   // local pointers
                TODO,                   // local x interval
                TODO,                   // local y interval
                TODO,                   // stride for index linearization
            );
