...
int main(int argc, char *argv[]) {
    ...
    int numRanks = 0;
    MPI_Comm_size(MPI_COMM_WORLD, TODO);
    int rank = 0;
    MPI_Comm_rank(MPI_COMM_WORLD, TODO);
    ...
    // initialize patches
    int numPatches = TODO;  // one patch per rank
    checkCudaError(cudaSetDevice(deviceId));
    ...
    initTemperaturePatch(patch.localU,
        TODO,       // global index space
        TODO,       // local index space
        TODO        // global offset for this patch
    );
    ...
    auto haloExchange = [&]() {
        ...
        if (rank > 0) {
            auto recvPtr = &patch.d_localUNew[TODO];    // bottom halo of current patch
            auto sendPtr = &patch.d_localUNew[TODO];    // first inner row of current patch
            MPI_Irecv(
        ...
        if (rank < numRanks - 1) {
            auto recvPtr = &patch.d_localUNew[TODO];   // top halo of current patch
            auto sendPtr = &patch.d_localUNew[TODO];   // last inner row of current patch
            MPI_Irecv(
                recvPtr,                // destination pointer
                patch.localNumCellsX,   // number of elements: one row
                MPI_DOUBLE,             // datatype
